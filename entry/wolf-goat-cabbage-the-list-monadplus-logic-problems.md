...
> Originally posted by [Justin Le](http://home.jle0.com:4111/) on December 26, 2013.
> [Read online!](http://home.jle0.com:4111/entry/wolf-goat-cabbage-the-list-monadplus-logic-problems.html)
...
4.  A simple type synonym; a `Plan` is just a list of `Move`s. Note that we are
    not using this list as a MonadPlus — it’s just a plain dumb list of moves in
    our plan.
...
2.  We declare a new type `Move` which is just a wrapper around a `Character`.
    ...
    (`λ:` represents a command at the interactive prompt ghci, and `:t` asks for
    the type of whatever comes after it)
...
Haskell is a functional language…but that “do” block sure looks very imperative
to me. What gives?
...
We use `[Farmer ..]` as shorthand for `[Farmer, Wolf, Goat, Cabbage]` — this is
because `Character` is an Enum, so it can be enumerated using enumeration
...
will put 6 (`3 * 2`) into `x` — it’ll take out the 3 and then apply `(*2)` to it
before storing it in `x`.
...
just not with their consecutive chaining, like MonadPlus. It deals with their
parallel choices, actually, as the name implies. This functionality is
...
[^1]: You might be aware that in the current Haskell standard library
    ...
    functionality — the “Plus”. We won’t be focusing on this part, because it is
    commonly regarded that it is more of a characteristic of the *Alternative*
