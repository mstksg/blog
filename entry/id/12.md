...
> [Read online!](http://home.jle0.com:4111/entry/practical-fun-with-monads-introducing-monadplus.html)
...
C++ — and intermediate Haskell users — people who have a somewhat firm grasp on
monads, but want to know about monads on a broader context (in particular, the
MonadPlus typeclass).
...
to `x`…do `halve x` and assign the result (8) to `y`…” — but remember, it’s
...
building up either a success or a failure…and if at any point I fail, the whole
thing is a failure”.
...
are some subtle differences — typeclasses are in general more powerful of a tool
than interfaces — but the two concepts provide similar roles in their respective
languages.
</aside>
...
Unfortunately, it’s not so simple — it needs to be implemented such that if your
health ever dips below 0, you are dead. Forever. No powerups will ever help you.
...
`Just 1` — the 1. So `h4` is supposed to represent the number inside its value,
too. But because `hit h3` results in `Nothing`; `Nothing` has no value “inside”,
so `h4` doesn’t even have a value! So obviously it doesn’t even make sense to
call `powerup h4`…therefore `h5` has no value either! It’s therefore meaningless
to call `powerup h5`, so meaningless to `return h6`…the entire thing is a
...
-   There is a common vocabulary for talking about MonadPlus concepts — “return”
    means “succeed with this value”, and “mzero” means “fail now”.
...
[^1]: I have to give a fair disclaimer here. MonadPlus, as it is currently
    ...
    Haskell standard library — the infamous [monad-applicative-functor hierarchy
